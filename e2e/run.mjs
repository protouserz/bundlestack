...
if (process.env.E2E_STORE_URL) {
  console.log(`Storefront → ${process.env.E2E_STORE_URL}`);
  const password = (process.env.E2E_STOREFRONT_PASSWORD || "").trim();
  if (!password) {
    console.warn(
      "Warning: E2E_STOREFRONT_PASSWORD is empty — storefront tests will be skipped until you set it in .env.e2e.",
    );
  }
}
